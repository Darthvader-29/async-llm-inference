...
- ✔ **Row-failure ownership (processor↔broker):** on pipeline failure the processor **re-raises and leaves the row `RUNNING`** (it does NOT `mark_failed`); the Phase 5 consumer owns the terminal transition — `requeue()` (RUNNING→PENDING) on transient-with-budget, `mark_failed()` (RUNNING→FAILED) on DLQ. Both require `RUNNING`, so a processor that marked the row terminal would make the broker raise `InvalidTransition` and break retry/DLQ. (Corrects the phase-7 doc §6.3 sample, which marked FAILED in the processor.) Guarded by `tests/unit/broker/test_processor_broker_contract.py`. Unexpected (non-`UpstreamError`) exceptions are wrapped as `PermanentUpstreamError` so they DLQ rather than retry forever.
